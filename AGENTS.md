...
This file serves as Codex's working memory for the RMWinslow.github.io project.
Rather than relying on the separate `.Codex/` memory system, store observations,
...
- **Memories go here.** Any context, observations, or learned details about the
  ...
  into this file rather than (or in addition to) the `.Codex/` memory directory.
...
- [x] Review the broken internal links identified in `claude_audits.md` for
  ...
  responsibility have been transferred to its `AGENTS.md`.
